...
public class HeatPumpFmuOptionModelCalculation extends AbstractOptionModelCalculation {
	...
	private FmuSimulationWrapper simulationWrapper;
	...
	public AbstractInterfaceFlow getEnergyOrGoodFlow(TechnicalSystemStateEvaluation tsse, TechnicalInterface ti, boolean isManualConfiguration) {
		...
		if (tsse!=this.tsseLastCalculated) {
			...
			double pElHeatPumpKW = this.getSimulationWrapper().getFmuSimulation().read("Pel_HP").asDouble();
			double pElCoilKW = this.getSimulationWrapper().getFmuSimulation().read("Pel_HP").asDouble();
